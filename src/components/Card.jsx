function Card({ title, actionLabel, onAction, children, compact = false, dense = false, minH, className = '', actionButtonClassName, actionButtonStyle }) {
  ...
  const buttonClass = dense
    ? 'px-2 py-0.5 text-[11px] rounded-md border border-[var(--border)] bg-[var(--surface)] hover:bg-gray-50 dark:hover:bg-gray-800'
    : compact
    ? 'px-2 py-1 text-xs rounded-md border border-[var(--border)] bg-[var(--surface)] hover:bg-gray-50 dark:hover:bg-gray-800'
    : 'px-3 py-1 rounded-md border border-[var(--border)] bg-[var(--surface)] hover:bg-gray-50 dark:hover:bg-gray-800'
  const bodyClass = dense ? 'p-1.5 flex-1' : compact ? 'p-2 flex-1' : 'p-3 flex-1'

  return (
    <section className={`bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm flex flex-col min-h-0 ${className}`}>
